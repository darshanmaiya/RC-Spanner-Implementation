...
public class Server {
	...
	private int							id;
	private List<ServerInfo>				participants;
	private ServerInfo 						leader;
	private String 						ipAddress;
	private int							port;
	private ServerSocket				serverSocket;
	private Socket 						redisYcsbSocket;
	private ServerInfo 						redisYcsbServer;
	...
	public void setLeader(ServerInfo leader) {this.leader = leader;}
	public List<ServerInfo> getParticipants(){ return this.participants; }
	...
	public Server(int id){
		...
		this.maxRound = 0;
		this.minProposal = 0;
		this.participants = new ArrayList<ServerInfo>();
	}
	...
	public void start(){
		...
		if (this.state == "LEADER"){
			try {
				...
				for (ServerInfo acceptor : this.participants){
				...
				while (true){
					...
					while ((ycsbMessage = ycsbReader.readLine()) != null){
						...
						numPromises = 0;
						int majority = (this.getParticipants().size()/2) + 1;
				...
				while (true)
				{
					...
					if (leaderMessage.contains("ACCEPT")){
						...
						if (receivedMaxRound >= this.minProposal){
							this.minProposal = receivedMaxRound;
							// "COMMIT=X:2 Y:3 Z:4"
							jedis.hset("ipAddress" + this.id, "", this.ipAddress);
